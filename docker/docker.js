...
const SCRIPT_COMMANDS = {
  ...
  'generate:wiki:with-codelists': 'python scripts/ontology_cli.py generate wiki --include-codelists',
  'generate:wiki:verbose': 'python scripts/ontology_cli.py generate wiki --verbose',
...
function run(scriptName, ...args) {
  ...
  if (!command) {
    ...
    console.error('Available scripts:', Object.keys(SCRIPT_COMMANDS).join(', '));
    process.exit(1);
  }
  
  // Special handling for validate:shacl - convert positional args to flags
  if (scriptName === 'validate:shacl' && args.length >= 2 && !args[0].startsWith('-')) {
  ...
  if (scriptName === 'convert:shacl' && args.length >= 2 && !args[0].startsWith('-')) {
  ...
  if (scriptName === 'convert:ts' && args.length >= 2 && !args[0].startsWith('-')) {
