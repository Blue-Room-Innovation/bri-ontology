...
if (!result.ok) {
    ...
    const itemErrors = result.errors.filter((e) => e.instancePath.includes('eventList/'));
    const topErrors = result.errors.filter((e) => !e.instancePath.includes('eventList'));
    console.error("Validation failed — top-level errors:", topErrors.length);
    topErrors.slice(0, 5).forEach((e) => console.error("  ", e.instancePath, e.schemaPath, e.message));
    console.error("Item-level errors:", itemErrors.length);
    itemErrors.slice(0, 20).forEach((e) => console.error("  ", e.instancePath, e.schemaPath, e.message));
    process.exit(1);
}
