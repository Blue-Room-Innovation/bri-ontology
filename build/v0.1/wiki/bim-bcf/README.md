...
classDiagram
   ...
   orthogonalCamera --> direction : cameraDirection
   perspectiveCamera --> direction : cameraDirection
   orthogonalCamera --> direction : cameraUpVector
   perspectiveCamera --> direction : cameraUpVector
   orthogonalCamera --> point : cameraViewPoint
   perspectiveCamera --> point : cameraViewPoint
   visualizationInfo --> clippingPlane : clippingPlanes
   components --> coloringEntry : coloring
   topic --> comment : comments
   visualizationInfo --> components : components
   visualizationInfo --> component : components
   coloringEntry --> components : components
   coloringEntry --> component : components
   ...
   clippingPlane --> point : location
   bitmap --> point : location
...
|<span id="bimSnippet">bimSnippet</span>|BIM snippet metadata containing reference and schema details.|[isExternal](#isExternal), [reference](#reference), [referenceSchema](#referenceSchema)|||
|<span id="bitmap">bitmap</span>|Bitmap overlay with placement, orientation, format, and size.|[format](#format), [height](#height), [reference](#reference)|[location](#location), [normal](#normal), [up](#up)||
|<span id="clippingPlane">clippingPlane</span>|Clipping plane defined by location and direction.||[direction](#direction), [location](#location)||
|<span id="coloringEntry">coloringEntry</span>|Single color rule entry linking components and a color value.|[colorValue](#colorValue)|[components](#components)||
|<span id="comment">comment</span>|Comment resource with authoring and modification metadata.|[author](#author), [commentText](#commentText), [date](#date), [modifiedAuthor](#modifiedAuthor), [modifiedDate](#modifiedDate)|[viewpoint](#viewpoint)||
...
|<span id="direction">direction</span>|3D direction vector represented by X, Y, and Z components.|[x](#x), [y](#y), [z](#z)|||
|<span id="document">document</span>|Document metadata entry with filename, description, and GUID.|[description](#description), [filename](#filename)|||
|<span id="documentReference">documentReference</span>|Document reference class for either internal document GUID or external URL.|[description](#description), [documentGuid](#documentGuid), [url](#url)|||
...
|<span id="file">file</span>|Referenced file metadata class in the markup header.|[date](#date), [filename](#filename), [ifcProject](#ifcProject), [ifcSpatialStructureElement](#ifcSpatialStructureElement), [isExternal](#isExternal), [reference](#reference)|||
...
|<span id="orthogonalCamera">orthogonalCamera</span>|Orthogonal camera definition.|[aspectRatio](#aspectRatio), [viewToWorldScale](#viewToWorldScale)|[cameraDirection](#cameraDirection), [cameraUpVector](#cameraUpVector), [cameraViewPoint](#cameraViewPoint)||
|<span id="perspectiveCamera">perspectiveCamera</span>|Perspective camera definition.|[aspectRatio](#aspectRatio), [fieldOfView](#fieldOfView)|[cameraDirection](#cameraDirection), [cameraUpVector](#cameraUpVector), [cameraViewPoint](#cameraViewPoint)||
|<span id="point">point</span>|3D point represented by X, Y, and Z coordinates.|[x](#x), [y](#y), [z](#z)|||
...
|<span id="topic">topic</span>|Core issue/topic class with metadata, workflow state, comments, and viewpoints.|[assignedTo](#assignedTo), [creationAuthor](#creationAuthor), [creationDate](#creationDate), [description](#description), [dueDate](#dueDate), [index](#index), [labels](#labels), [modifiedAuthor](#modifiedAuthor), [modifiedDate](#modifiedDate), [priority](#priority), [referenceLinks](#referenceLinks), [serverAssignedId](#serverAssignedId), [stage](#stage), [title](#title), [topicStatus](#topicStatus), [topicType](#topicType)|[bimSnippet](#bimSnippet), [comments](#comments), [documentReferences](#documentReferences), [relatedTopics](#relatedTopics), [viewpoints](#viewpoints)||
|<span id="version">version</span>|Version payload with required version identifier.|[versionId](#versionId)|||
|<span id="viewPoint">viewPoint</span>|Viewpoint resource with viewpoint file, snapshot, index, and GUID.|[index](#index), [snapshot](#snapshot), [viewpointFile](#viewpointFile)|||
...
|<span id="visualizationInfo">visualizationInfo</span>|Visualization payload including camera configuration and overlays.||[bitmaps](#bitmaps), [clippingPlanes](#clippingPlanes), [components](#components), [lines](#lines), [orthogonalCamera](#orthogonalCamera), [perspectiveCamera](#perspectiveCamera)||

## Data Properties

|Name|Description|Domain|Range|Subproperty of|
| :--- | :--- | :--- | :--- | :--- |
|<span id="aspectRatio">aspectRatio</span>|Width/height aspect ratio of the camera view.|[orthogonalCamera](#orthogonalCamera), [perspectiveCamera](#perspectiveCamera)|double||
...
|<span id="date">date</span>|Timestamp associated with a file or comment entry.|[file](#file), [comment](#comment)|dateTime||
...
|<span id="description">description</span>|Human-readable description text.|[document](#document), [topic](#topic), [documentReference](#documentReference)|string|description, description|
...
|<span id="filename">filename</span>|Filename associated with a document or file entry.|[document](#document), [file](#file)|string|name|
...
|<span id="index">index</span>|Integer sort/index value for topic or viewpoint.|[topic](#topic), [viewPoint](#viewPoint)|int|position|
|<span id="isExternal">isExternal</span>|Indicates whether a referenced resource is external to the BCF package.|[bimSnippet](#bimSnippet), [file](#file)|boolean||
|<span id="labels">labels</span>|Label attached directly to a topic.|[topic](#topic)|string|keywords, subject|
|<span id="modifiedAuthor">modifiedAuthor</span>|Author of the latest modification.|[topic](#topic), [comment](#comment)|string|author|
|<span id="modifiedDate">modifiedDate</span>|Timestamp of the latest modification.|[topic](#topic), [comment](#comment)|dateTime|dateModified, modified|
...
|<span id="reference">reference</span>|Reference string (filename, URI, or path) used by snippets, files, and bitmaps.|[bimSnippet](#bimSnippet), [file](#file), [bitmap](#bitmap)|string|contentUrl|
...
|<span id="x">x</span>|X coordinate component.|[point](#point), [direction](#direction)|double||
|<span id="y">y</span>|Y coordinate component.|[point](#point), [direction](#direction)|double||
|<span id="z">z</span>|Z coordinate component.|[point](#point), [direction](#direction)|double||
...
|<span id="cameraDirection">cameraDirection</span>|Camera forward direction vector.|[orthogonalCamera](#orthogonalCamera), [perspectiveCamera](#perspectiveCamera)|[direction](#direction)||
|<span id="cameraUpVector">cameraUpVector</span>|Camera up direction vector.|[orthogonalCamera](#orthogonalCamera), [perspectiveCamera](#perspectiveCamera)|[direction](#direction)||
|<span id="cameraViewPoint">cameraViewPoint</span>|Camera position in 3D space.|[orthogonalCamera](#orthogonalCamera), [perspectiveCamera](#perspectiveCamera)|[point](#point)||
...
|<span id="components">components</span>|Components definition containing selection, visibility, and coloring sections. Links visualization info or a coloring entry to components according to the SHACL model.|[visualizationInfo](#visualizationInfo), [coloringEntry](#coloringEntry)|[components](#components), [component](#component)||
...
|<span id="location">location</span>|3D location used by clipping planes and bitmap placement.|[clippingPlane](#clippingPlane), [bitmap](#bitmap)|[point](#point)||
