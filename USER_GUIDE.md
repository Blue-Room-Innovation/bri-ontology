...
- The file `config.yml` is the **single source of truth** for versions, paths, enabled artifacts, validation scenarios, and conversion pipelines.
- Most commands run via Docker (`node docker/docker.js ...`) and **read `config.yml` at runtime**. Changing versions or adding scenarios is normally done by editing `config.yml`, not by modifying scripts.
...
- `paths.*` (where each component lives in the repo)

**Ontology & shapes inventory**

- `ontologies[]`: list of ontology files (name/file/metadata)
- `shapes[]`: list of SHACL files (name/file + ontology association)

**Validation**

- OWL: `validation.owl.reasoner`, `validation.owl.profile`, `validation.owl.include_codelists`
- SHACL: `validation.shacl.scenarios.*` (data/shapes/format/extras)

**Conversion & generation**

- `generation.artifacts[]`: which SHACL shapes should produce JSON Schema + TypeScript
- `conversion.shacl_to_json.*` and `conversion.json_to_ts.*`: named conversions with concrete input/output paths

**Wiki & release**

- `wiki.*` controls documentation generation
- `release.*` describes versioning/release strategy (used by release scripts)
...
- New version: bump `*_version` and add the new version folders/files.
- New SHACL validation scenario: add under `validation.shacl.scenarios`.
- New generated artifact (schema/types): add under `generation.artifacts` (and optionally `conversion.*`).
...
**Configuration used (`config.yml`)**:

- Inputs are discovered from `paths.ontology` + `ontology_version` and the `ontologies[]` list (unless overridden via CLI options).
- Reasoning/profile come from `validation.owl.reasoner` and `validation.owl.profile`.
- Build output goes to `paths.build` + `build_version`.
...
- Mirrors `validation.owl.include_codelists: true` (and discovers codelists from `paths.codelists` + `codelists_version`).
...
- Lists `validation.shacl.scenarios.*` (any "default" marker is informational only).
...
```bash
# Validates all scenarios configured in config.yml
npm run validate:shacl
```

**Configuration used (`config.yml`)**:

- Iterates over `validation.shacl.scenarios.*` and validates each scenario's `data`, `shapes`, `format`, and `extras`.
...
- Resolves scenario values from `validation.shacl.scenarios.<name>`.
...
- Scenario values are used as defaults, but any CLI flag (`-d`, `-s`, `-f`, `-e`) overrides the scenario.
...
- Organize scenarios for CI/CD pipelines (e.g., smoke vs full)
...
- Uses `generation.artifacts[]` to decide which shapes to process.
- Reads shapes from `paths.shapes` + `shapes_version`.
- Writes artifacts under `paths.build` + `build_version`.
- If present, conversion mappings in `conversion.shacl_to_json` and `conversion.json_to_ts` define concrete input/output locations.
...
- Output directory comes from `wiki.output_dir` (defaults to `docs/wiki` in this repo).
- If codelists are included, it follows `wiki.include_codelists` (and uses `paths.codelists` + `codelists_version`).
- Ontologies/shapes inventory comes from `ontologies[]` / `shapes[]` and the `*_version` + `paths.*` settings.
...
- Equivalent to running wiki generation with `wiki.include_codelists: true`.
...
- You can run conversions ad-hoc (explicit `-i/-o`) or define repeatable named conversions under `conversion.shacl_to_json.*`.
- Default versioned paths usually follow `paths.shapes` + `shapes_version` and `paths.build` + `build_version`.
...
- You can run conversions ad-hoc (explicit `-i/-o`) or define repeatable named conversions under `conversion.json_to_ts.*`.
- The default banner/source metadata typically points back to SHACL via `conversion.json_to_ts.<name>.source`.
...
- Same as TypeScript generation (`generation.artifacts`, `*_version`, `paths.*`).
...
- OWL stage uses `validation.owl.*` + versioned `paths.*`.
- Types stage uses `generation.*` / `conversion.*` + versioned `paths.*`.
- Wiki stage uses `wiki.*` + ontologies/shapes inventory + versioned `paths.*`.
...
npm run validate:shacl            # SHACL validation (runs all scenarios)
