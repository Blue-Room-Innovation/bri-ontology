...
The idea and best practice here is to use an existing Shape, for example the one for [recycling](https://blue-room-innovation.github.io/bri-ontology/build/v0.1/recycling.context.jsonld). The reason is simple: the work of defining the JSON-LD structure and the assigned semantics has already been done beforehand, so you will not make mistakes. You could also create the `@context` yourself—it is more flexible—but for context (no pun intended).
...
1. If you want to transform a JSON into JSON-LD, do the following:
   - Create a dictionary where `@context` points to `https://blue-room-innovation.github.io/bri-ontology/build/vX.X/X.context.jsonld`
   - Set `@id` and `@type`
...
```jsonld
{
  "@context": "https://blue-room-innovation.github.io/bri-ontology/build/v0.1/recycling.context.jsonld",
  "@graph": [
    {
      "@id": "E-2222.22",
      "@type": "Recycler",
...
1. Associate the context via an HTTP header
   ...
   - In the HTTP response, you add a `Link` header pointing to the context
     ...
     `Link: <https://blue-room-innovation.github.io/bri-ontology/build/vX.X/X.context.jsonld>; rel="http://www.w3.org/ns/json-ld#context"; type="application/ld+json"`
