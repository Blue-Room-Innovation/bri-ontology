...
4. If it is valid, modify [[3. Configuration|config.yaml]] and configure the new auto-generated outputs:
...
6. Generate the JSON Schema from the Shapes: `npm run convert:shacl`
...
- Besides building the package, this also updates the generated typings artifacts defined in `config.yaml` (`generation.artifacts`) and the `index.ts` files.
...
- Review it and check that it is correct.
...
The idea and best practice here is to use an existing Shape, for example the one for [recycling](https://raw.githubusercontent.com/Blue-Room-Innovation/bri-ontology/main/build/v0.1/recycling.context.jsonld). The reason is simple: the work of defining the JSON-LD structure and the assigned semantics has already been done beforehand, so you will not make mistakes. You could also create the `@context` yourself—it is more flexible—but for context (no pun intended).
...
1. If you want to transform a JSON into JSON-LD, do the following:
   - Create a dictionary where `@context` points to `https://raw.githubusercontent.com/Blue-Room-Innovation/bri-ontology/main/build/v0.1/X.context.jsonld`
   - Set `@id` and `@type`
...
1. Associate the context via an HTTP header
   ...
   - In the HTTP response, you add a `Link` header pointing to the context
     ...
     `Link: <https://raw.githubusercontent.com/Blue-Room-Innovation/bri-ontology/main/build/v0.1/X.context.jsonld>; rel="http://www.w3.org/ns/json-ld#context"; type="application/ld+json"`
...
2. Find the corresponding example in the `bri-ontology` repository and temporarily paste the content there.
